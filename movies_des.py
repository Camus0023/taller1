...
for i in range(50):
    prompt =  f"{instruction} Has una descripción de la película {movies[i]['_Title']}"
    response = get_completion(prompt)
    movies[i]['description'] = response 
    prompt = f"{instruction_genre} Género de la película {movies[i]['_Title']}"
    response = get_completion(prompt)
    movies[i]['genre'] = response   
    prompt = f"{instruction_year} Año de lanzamiento de la película {movies[i]['_Title']}"
    response = get_completion(prompt)
    movies[i]['year'] = response
    print(movies[i]['_Title'])
